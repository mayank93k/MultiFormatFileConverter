...
object dataFrameWriter {
   ...
   * @param option         writeOptions is optional (User should not send any writeOptions param
   *                       if it is not required) default is Map.empty
   */
  def dataFrameWriter(dataframe: DataFrame, noOfPartitions: Int = 1, destFileType: String, saveMode: String = "append",
                      path: String, option: Map[String, String] = Map.empty): Unit = {
