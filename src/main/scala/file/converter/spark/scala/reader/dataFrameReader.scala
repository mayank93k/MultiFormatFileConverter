...
object dataFrameReader extends DataTypes {
   ...
   * @param option       readOptions is optional (User should not send any readOption param if it is not required) default is Map.empty
   * @return Returns the source dataframe
   */
  def dataFrameReader(sparkSession: SparkSession, path: String, srcFileType: String, fileSchema: DataTypes = null, option: Map[String, String] = Map.empty): DataFrame = {

    fileSchema match {
      case stringFileSchema(stringSchema) => sparkSession.read.format(srcFileType).schema(stringSchema).options(option).load(path)
      case structFileSchema(structSchema) => sparkSession.read.format(srcFileType).schema(structSchema).options(option).load(path)
